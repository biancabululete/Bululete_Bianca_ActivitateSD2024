...
void inserareLocuinta(struct Locuinta** vector, int* dim, struct Locuinta l)
{
	...
	{
		copie[i] = (*vector)[i]; //shallow copy (nu facem stergere a zonei de memorie dupa)
	}
	copie[(*dim) - 1] = l;
	if (*vector != NULL)
	{
		...
		if (f != NULL) {
			*dim = 0;
			...
			while (fgets(buffer, 100, f)!=NULL)
			{
				...
				locuinta.id = atoi(token); //cast la int = a to int (daca avem float: atof = a to float)
...
{
	printf("ID-ul: %d\tStrada %s numarul %d\n", l.id, l.strada, l.numar);
}
